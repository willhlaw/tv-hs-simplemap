# tv-hs-simplemap
Custom map for SimpleMap Wordpress for Tidewater Virginia Historical Society.

[Development site](http://tv-hs-simplemap.herokuapp.com/?page_id=12) built with [Heroku WP](https://github.com/xyu/heroku-wp) thanks to [xyu](https://github.com/xyu)

The customizations are located on the [production branch](https://github.com/willhlaw/tv-hs-simplemap/tree/production).

Features
--------
- Add an address from an infowindow for the first time (it'll disappear after the starting address is established).
- Click on a second infowindow and have the ability to "Add To Trip".
- Directions are set to optimize the waypoints.
- Alternative routes will show at the top of the direction results if there are multiple paths.
- Street input field has been changed to Address and the rest of the fields are expected to be hidden via css. 

Versions
--------
- Wordpress 4.2.2
- SimpleMap Wordpress plugin 2.7
- Other semvers defined in [composer.json](https://github.com/willhlaw/tv-hs-simplemap/blob/production/composer.json)

Steps to Customize
------------------
1. Copy classes/simplemap.php [here](https://github.com/willhlaw/tv-hs-simplemap/blob/production/public/wp-content/plugins/simplemap/classes/simplemap.php) to same location in your Wordpress directory.
2. Install SimpleMap plugin
3. Export/Import simplemap.csv or add location and categories manually

Steps to Build Development Environment
--------------------------------------
1. Follow instructions below for Heroku WP to get WordPress running on Heroku.
2. Follow the Steps to Customize above.
